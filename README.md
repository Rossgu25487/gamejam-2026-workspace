...
GitHub 文件列表中间一列显示的是各项最近的提交说明；一次修改涉及多个目录时，会重复显示同一句。目录用途见下表。

| 目录 | 用途 |
|---|---|
| `game/` | Godot 游戏工程 |
| `planning/` | 已入库的策划资料与当前依据 |
| `docs/` | 使用、同步、接口与交接说明 |
| `workspace/` | 岗位名单、项目节点和本机续接线索 |
| `site/`、`tools/` | 网页工作台、构建和检查工具 |
| `qa/`、`release/` | 测试与交包资料 |
| `岗位工作流/`、`分发包/` | 已分发的岗位参考与归档 |

## 怎样开始

1. 打开工作台，选择本人岗位；筛选条件保留在地址中，也可记住本机岗位。
...
当前为命题前准备，题材和正式制作任务待公布。题前学习与准备单独计；2026 年 10 月 1—21 日正式制作每人以 **12 小时为精简基线，必要时可扩大，累计最多 24 小时**。扩大时在任务中记录估时与本人可投入时间，实际工时由本人确认；协调、制作、验证、交包与返修均计入本人额度，24 小时是上限，不要求用满。
...
工作台界面可在本地构建后通过 `node tools/preview-dashboard.mjs` 预览。预览与本地提交均不发布到 GitHub，上传仍需对本次内容明确授权。
